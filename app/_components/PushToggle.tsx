...
/**
 * Every state says what is true and what to do about it. A control that
 * silently does nothing is the failure mode this component exists to avoid —
 * especially on iOS, where push is impossible until the app is on the Home
 * Screen, and when signed out, where the mutation would simply reject.
 *
 * `nested` drops the icon and title for use inside the install page's numbered
 * step, which already supplies both.
 */
export function PushToggle({
  ...
  const needsInstallFirst =
    os === "ios" && verdict !== "installed" && verdict !== "pending";
  const signedOut = !authLoading && !isAuthenticated;
  // Fan-out only reaches admins, so offering anyone else a working switch would
  // subscribe them to notifications that can never arrive.
  const notAnAdmin = isAuthenticated && me !== undefined && me?.role !== "admin";
  ...
  const body = (
    <>
      ...
      {error && (
        // Push errors can carry long unbroken tokens; break them rather than
        // pushing the card sideways on a narrow screen.
        <p className="mt-3 flex items-start gap-1.5 text-sm break-words text-amber-700 dark:text-amber-500">
  ...
  if (nested) return <div className={className}>{body}</div>;
...
function statusLine({
  ...
  if (signedOut) return "Sign in as an admin to get alerts on this device.";
  if (notAnAdmin)
    return "Review alerts go to site admins. Your account doesn't have admin access.";
